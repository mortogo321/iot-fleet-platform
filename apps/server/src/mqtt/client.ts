 ...
 * `reconnectPeriod` is a fixed interval, so automatic reconnection is disabled and driven
 * manually from the `close` event instead.
 */
export function connectMqtt(options: ConnectMqttOptions): PlatformMqttClient {
  ...
  function scheduleReconnect(): void {
    ...
    reconnectTimer = setTimeout(() => {
      reconnectTimer = undefined;
      if (!intentionalEnd) client.reconnect();
    }, backoffMs);
  ...
  client.on('message', (topic, payload) => routeMessage(topic, payload, options.handlers));
